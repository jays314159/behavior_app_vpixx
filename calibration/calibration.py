...
class Fsm_calThreadSignals(QObject):
    to_main_thread = pyqtSignal(object)

class Fsm_calThread(QRunnable):
    '''
    Some variables in this thread (e.g., 'self.fsm_parameter['run']') are not nominally thread-safe, 
    but it's practically OK, due to Python's GIL.
    '''
    def __init__(self, fsm_to_screen_sndr):
        ...
        self.signals = Fsm_calThreadSignals()
            ...
            while self.fsm_parameter['run']:
                ...
                if state == 'STR_TARGET_PURSUIT':
                    if self.fsm_parameter['is_pursuit_tgt']:
                        self.fsm_to_screen_sndr.send(('tgt','draw',pursuit_v*(self.t-state_start_time) + pursuit_start_p))
...
class Fsm_calGui(FsmGui):
    def __init__(self,cal_name,fsm_to_screen_sndr, stop_fsm_process_Event):
        ...
        super(Fsm_calGui,self).__init__(self.fsm_to_screen_sndr, self.stop_fsm_process_Event)
        ...
        # Load parameters
        self.cal_parameter, self.parameter_file_path = lib.load_parameter(self.cal_name,'cal_parameter.json',True,lib.set_default_cal_parameter,'calibration')
        ...
        try:
            ...
            for tgt_idx in range(self.num_cal_tgt):
                ...
                self.cal_parameter['tgt_'+str(tgt_num)] = [self.tgt_widgets_dict['tgt_'+str(tgt_num)+'_horz_QDoubleSpinBox'].value(),
                                                                      self.tgt_widgets_dict['tgt_'+str(tgt_num)+'_vert_QDoubleSpinBox'].value(),
                                                                      self.tgt_widgets_dict['tgt_'+str(tgt_num)+'_QCheckBox'].isChecked()]
            all_parameter[self.cal_name] = self.cal_parameter    
...
class Fsm_calProcess(multiprocessing.Process):
    ...
    def run(self):  
        ...
        fsm_app_gui = Fsm_calGui(self.cal_name,self.fsm_to_screen_sndr, self.stop_fsm_process_Event)
        fsm_app_gui.show()
