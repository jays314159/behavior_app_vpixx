...
class OptoSimpleSacFsmProcess(multiprocessing.Process):
    ...
    def run(self):
        ...
        while not self.stop_fsm_process_Event.is_set():
            if not self.stop_exp_Event.is_set():
                ...
                #init stim params
                avg_reaction_time = fsm_parameter['avg_reaction_time']/1000
                stim_window = fsm_parameter['stim_window']/1000
                stim_length = fsm_parameter['stim_length']/1000
                # Create target list
            ...
            while not self.stop_fsm_process_Event.is_set() and run_exp:
                ...
                while not self.stop_fsm_process_Event.is_set() and run_exp:
                    ...
                    if state == 'INIT':
                        ...
                        stim_on_time = -1
                        dout_ch_7 = 0
                        stim_trial = np.random.binomial(1,fsm_parameter['stim_prob'])
                    ...
                    if state == 'DETECT_SACCADE_START':
                        ...
                        if stim_trial and ((self.t - stim_on_time) >= stim_length) and (stim_on_time != -1):
                            # print('start timer')
...
class OptoSimpleSacGuiProcess(multiprocessing.Process):
    def __init__(self, exp_name, fsm_to_gui_rcvr, gui_to_fsm_sndr, stop_exp_Event, stop_fsm_process_Event, real_time_data_Array, main_parameter, parent=None):
        super(OptoSimpleSacGuiProcess,self).__init__(parent)
        self.exp_name = exp_name
        self.fsm_to_gui_rcvr = fsm_to_gui_rcvr
        self.gui_to_fsm_sndr = gui_to_fsm_sndr
        self.stop_exp_Event = stop_exp_Event
