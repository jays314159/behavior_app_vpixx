...
class DelaySacFsmProcess(multiprocessing.Process):
            ...
            while not self.stop_fsm_process_Event.is_set() and run_exp: 
                ...
                while not self.stop_fsm_process_Event.is_set() and run_exp:
                    ...
                    if state == 'INIT':
                        ...
                        self.cue_ring.pos = [self.start_x, self.start_y]
                        self.cue_white_circ.pos = [self.start_x, self.start_y]
                        self.cue_ring_left.pos = [self.start_x,self.start_y]
                        
                        
                        # Set trial parameters
                        tgt_idx = random.randint(0,num_tgt_pos-1) # Randomly pick target
                        corr_idx = random.randint(0,num_corr_pos-1)
                        if fsm_parameter['dir_switch_interval'] > 0:
                            if fsm_parameter['num_tgt_display'] > 0:
                                tgt_idx = tgt_indx_dict[current_dir]
                        
                            cue_angle = tgt_directions[current_dir]*np.pi/180
                            cue_x = fsm_parameter['prim_sac_amp']*math.cos(cue_angle)
                            cue_y = fsm_parameter['prim_sac_amp']*math.sin(cue_angle)
                            cue_pos = np.array([cue_x,cue_y]) + np.array(start_pos)
                        else:
                            tgt_idx = random.randint(0,num_tgt_pos-1) # Randomly pick target
                        ...
                        sac_right = self.cue_x - self.start_x
                        ...
                        cues = ['arrow','landolt','circle','color']
                        ...
                        cue_duration = delay_time
                        delay_time = 0
                        pre_cue_duration = fsm_parameter['mask_duration']
                        display_cue = True
                        display_tgt = True
                        ...
                        if np.random.rand() < fsm_parameter['tgt_prob']:
                            reward_fixation = True
                            #cue_duration = cue_duration + fsm_parameter['post_delay'] # Hold fixation for longer if rewarded just for fixating
                        
                        if np.random.rand() > fsm_parameter['cue_probability']:
                            display_cue = False
                        elif np.random.rand() < fsm_parameter['keep_cue_on']:
                            keep_cue_on = True
                        ...
                        '''
                        if np.random.rand() < fsm_parameter['choice_prob'] and (dir_switch_interval == 0 or (trial_num % dir_switch_interval) > fsm_parameter['num_forced_beginning']):
                            num_tgt_display = max(fsm_parameter['num_tgt_display'] - 1,0)
                            if not reward_fixation:
                                self.num_choice += 1
                        else:
                            num_tgt_display = 0
                        '''
                        ...
                        transparent = False
                        no_cue_bool = False
                        if (dir_switch_interval == 0 or ((trial_num-1) % dir_switch_interval) >= fsm_parameter['num_forced_beginning']):
                            if (fsm_parameter['opacity'] < 0.999):
                                transparent = True
                            no_cue_bool = True
                             
                        if (fsm_parameter['tgt_opacity'] > 0.999) or dir_switch_interval == 0 or ((trial_num-1) % dir_switch_interval) >= fsm_parameter['num_forced_beginning']:
                            tgt_trans_bool = False
                        else:
                            tgt_trans_bool = True
                        print(f'transparent: {transparent}')
                        '''
                        if fsm_parameter['num_forced_beginning'] > 0:
                            draw_distractor = (trial_num % dir_switch_interval) > fsm_parameter['num_forced_beginning']
                        else:
                        ...
                        num_tgt_display = 0
                        if np.random.rand() < fsm_parameter['choice_prob'] or reward_fixation:
                            num_tgt_display = max(fsm_parameter['num_tgt_display'] - 1,0)
                            '''if not reward_fixation:
                                if keep_cue_on:
                                    self.num_choice += 1
                        ...
                        self.arrow_shape.ori = np.rad2deg(np.arctan2(self.start_y - self.cue_y,self.cue_x - self.start_x))
                    ...
                    if state == 'STR_TARGET_FIXATION':
                        ...
                        if (self.t-state_inter_time) >= fsm_parameter['min_fix_time']:
                            ...
                            if pre_cue_duration > 1e-3:
                                state = 'DISPLAY_TGT'
                                print('state = DISPLAY_TGT')
                                 
                            elif (keep_cue_on or cue_duration > 1e-3):
                        ...
                        if not self.eye_blink:
                            ...
                            if display_tgt:
                                for counter_tgt in range(len(tgt_display_coords)):
                                    ...
                                    if counter_tgt in self.rm_inds:
                                        self.wrong_tgt_shape.pos = self.tgt.pos
                                        self.wrong_tgt_shape.draw()
                                    else:
                                        self.draw_tgt(tgt_symbol)
                            ...
                            self.trial_data['state_start_t_wait_for_cue'].append(self.t)
                            state = 'WAIT_FOR_CUE'
                            print('state = WAIT_FOR_CUE')
                            
                    if state == 'WAIT_FOR_CUE':
                        eye_dist_from_tgt = np.sqrt((self.start_x-self.eye_x)**2 + (self.start_y-self.eye_y)**2)
                        if eye_dist_from_tgt > fsm_parameter['fix_area']/2 or self.eye_blink:
                            state_start_time = self.t
                            state_inter_time = self.t
                            ...
                            print(f'state = INCORRECT_SACCADE, broke fixation, false_start_bool  = {false_start_bool}')
                            self.trial_data['state_start_t_incorrect_saccade'].append(self.t)
                            
                            self.pd_tgt.draw()
                            self.window.flip()
                            
                        if (self.t-state_inter_time) >= pre_cue_duration:
                            state_start_time = self.t
                            state_inter_time = self.t
                            self.trial_data['state_start_t_cue_display'].append(self.t)
                            state = 'DISPLAY_CUE'
                    ...
                    if state == 'DISPLAY_CUE': # Display all targets and cue simultaneously
                        if not self.eye_blink:
                            if display_cue:
                                self.draw_cue(cue_type,fsm_parameter['center_cue'],False,False,sac_right,self.fix_tgt.pos)
                                if fsm_parameter['show_cue_early']:
                    ...
                    if state == 'SACCADE_GO_CUE':
                        ...
                        if reward_fixation:
                            ...
                            if display_cue:
                                self.draw_cue(cue_type,fsm_parameter['center_cue'],transparent,False,sac_right,[self.cue_x,self.cue_y])
                            cue_on = True
                            ...
                            self.tgt_y = self.cue_y
                            self.tgt.pos = (self.tgt_x,self.tgt_y)
                            if fsm_parameter['num_tgt_display'] > 0 and (display_tgt or not display_tgt) :             
                                #self.tgt.draw()
                                self.draw_tgt(tgt_symbol,change_tgt=change_tgt,transparent = False)
                    ...
                    if state == 'DETECT_SACCADE_START':
                        if (self.t - state_start_time) >= fsm_parameter['post_delay'] and cue_on and not keep_cue_on: # Getting rid of cue after post delay
                            ...
                            self.tgt.pos = (self.tgt_x,self.tgt_y)
                            if fsm_parameter['num_tgt_display'] > 0 and display_tgt:             
                                self.draw_tgt(tgt_symbol,transparent=False)
                            '''
                            state_start_time = self.t
                            state_inter_time = self.t
                            self.trial_data['state_start_t_incorrect_saccade'].append(self.t)    
                    ...
                    if state == 'DETECT_SACCADE_END':
                        if (self.eye_speed < fsm_parameter['sac_on_off_threshold']) and (self.t-state_start_time > 0.005):#25):
                            ...
                            if (eye_dist_from_cue_tgt < fsm_parameter['rew_area']/2):
                                ...
                                if wrong_tgt_bool:
                                    #if counter_tgt not in self.rm_inds:
                                    #    self.rm_inds.append(remove_tgt_ind)
                                    self.wrong_tgt_shape.pos = tgt_display_coords[remove_tgt_ind]
                                    #self.wrong_tgt_shape.draw()
                                self.write_Dout(1,1)
                                self.window.flip()
                                if wrong_tgt_bool:
                    ...
                    if state == 'DELIVER_REWARD':
                        ...
                        if reward_fixation:
                            ...
                            self.draw_cue(cue_type,fsm_parameter['center_cue'],False,False,sac_right,[self.cue_x,self.cue_y])
                            self.draw_tgt(tgt_symbol,change_tgt=change_tgt)
                    ...
                    if state == 'TRIAL_SUCCESS':
                        ...
                        if (self.t-state_start_time) > fsm_parameter['ITI']:
                            ...
                            if dir_switch_interval > 0 and trial_num % int(dir_switch_interval) == 0:
                            ...
                            if cue_duration > 1e-3:
                                if num_choice > 0:
                                    if num_choice_no_cue > 0:
                                        self.fsm_to_gui_sndr.send(('log',datetime.now().strftime("%H:%M:%S") + '; trial num: ' + str(trial_num) + ' Delay Success: ' +str(self.num_delay - self.num_false_start)+'/'+str(self.num_delay)+' ('+str(int(100*(1-self.num_false_start/self.num_delay)))+'%), Choice Accuracy: ' + str(num_correct)+'/'+str(num_choice)+' ('+str(int(100*num_correct/num_choice))+'%), No Cue Accuracy: ' + str(num_correct_no_cue)+'/'+str(num_choice_no_cue)+' ('+str(int(100*num_correct_no_cue/num_choice_no_cue))+'%)'))
                                    else:
    ...
    def update_target(self,fsm_parameter):
        ...
        wrong_param,_ = lib.load_parameter('','tgt_parameter.json',True,False,lib.set_default_tgt_parameter,'wrong_tgt')
        ...
        shape = ((0, 0),(0.5,1),(1,0),(0.5,-1))
        self.arrow_shape = visual.shape.ShapeStim(win=self.window,lineColor=arrow_param['line_color'],fillColor=arrow_param['fill_color'],
                      lineWidth=arrow_param['line_width'],vertices=shape,
                      size = (arrow_param['cue_length'],arrow_param['cue_width']),units='deg')
        ...
        self.cue_ring_left = visual.Circle(win=self.window,size=circle_param['outer_size'],fillColor=circle_param['left_color'],units='deg', lineColor=circle_param['line_color'],lineWidth=circle_param['line_width'])
        
        print(circle_param['fill_color'])
        fill_color = np.array(circle_param['fill_color'])*fsm_parameter['opacity'] + (1-fsm_parameter['opacity'])*np.array([1.0,1.0,1.0])
        print(fill_color)
        fill_color_left = np.array(circle_param['left_color'])*fsm_parameter['opacity'] + (1-fsm_parameter['opacity'])*np.array([1.0,1.0,1.0])
        line_color = np.array(circle_param['line_color'])*fsm_parameter['opacity'] + (1-fsm_parameter['opacity'])*np.array([1,1,1])
        self.cue_transparent = visual.Circle(win=self.window,size=circle_param['outer_size'],fillColor=fill_color,units='deg', lineColor=line_color,lineWidth=circle_param['line_width'])
        ...
        self.cue_white_transparent =  visual.Circle(win=self.window,size=circle_param['inner_size'],fillColor=(1,1,1),units='deg', lineColor=line_color,lineWidth=circle_param['line_width'])
        self.cue_ring_left_trans = visual.Circle(win=self.window,size=circle_param['outer_size'],fillColor=fill_color_left,units='deg', lineColor=line_color,lineWidth=circle_param['line_width'])
        ...
        self.wrong_tgt_shape = visual.Rect(win=self.window, width=wrong_param['size'],height=wrong_param['size'], units='deg', 
                      lineColor=wrong_param['line_color'],fillColor=wrong_param['fill_color'],
                      lineWidth=wrong_param['line_width'])
        #self.pd_tgt.draw()
    ...
    def draw_cue(self,cue_type,center_cue,transparent,mask,sac_right,pos):
        if cue_type == 'arrow':
            if mask:
                self.arrow_mask.pos = self.cue.pos
                self.arrow_mask.draw()
            else:
                self.arrow_shape.pos = self.cue.pos
        ...
        elif cue_type == 'circle':
            self.cue_ring.pos = [self.cue_x,self.cue_y]
            self.cue_white_circ.pos = self.cue_ring.pos
            
            if transparent:
                self.cue_transparent.pos = self.cue_ring.pos
                self.cue_white_transparent.pos = self.cue_ring.pos
                self.cue_transparent.draw()
                self.cue_white_transparent.draw()
            else:
            	self.cue_ring.draw()
            	self.cue_white_circ.draw()
            #self.draw_cue('arrow',True,False)
        else:
            if sac_right >= 0:
                if transparent:
                    self.cue_transparent.pos = pos
                    self.cue_transparent.draw()
                else:
                    self.cue_ring.pos = pos
                    self.cue_ring.draw()
            else:
                if transparent:
                    self.cue_ring_left_trans.pos = pos
                    self.cue_ring_left_trans.draw()
                else:
                    self.cue_ring_left.pos = pos
                    self.cue_ring_left.draw()
                    
            if transparent:
                self.cue_white_transparent.pos = pos
                self.cue_white_transparent.draw()
            else:
                self.cue_white_circ.pos = pos
                self.cue_white_circ.draw()
    ...
    def init_trial_data(self):
        ...
        self.trial_data['state_start_t_wait_for_cue'] = []
        ...
        self.trial_data['display_cue'] = []
        self.trial_data['display_tgt'] = []
...
class DelaySacGui(FsmGui):
    ...
    def init_signals(self):
        ...
        self.keep_cue_on_QDoubleSpinBox.valueChanged.connect(self.keep_cue_on_QDoubleSpinBox_valueChanged)
        self.include_corr_sac_QCheckBox.stateChanged.connect(self.include_corr_sac_QCheckBox_stateChanged)
        
        self.show_cue_early_QCheckBox.stateChanged.connect(self.show_cue_early_QCheckBox_stateChanged)
        ...
        self.opacity_QDoubleSpinBox.valueChanged.connect(self.opacity_QDoubleSpinBox_valueChanged)
    ...
    def random_tgt_QCheckBox_stateChanged(self):
        ...
        self.save_QPushButton.setStyleSheet('background-color: #FFCC00')
    @pyqtSlot()
    def show_cue_early_QCheckBox_stateChanged(self):
        self.exp_parameter['show_cue_early'] = self.show_cue_early_QCheckBox.isChecked()
        self.save_QPushButton.setStyleSheet('background-color: #FFCC00')
    ...
    def num_cue_dir_QDoubleSpinBox_valueChanged(self):
        ...
        self.save_QPushButton.setStyleSheet('background-color: #FFCC00')
    @pyqtSlot()
    def keep_cue_on_QDoubleSpinBox_valueChanged(self):
        self.exp_parameter['keep_cue_on'] = self.keep_cue_on_QDoubleSpinBox.value()
        self.save_QPushButton.setStyleSheet('background-color: #FFCC00')
    @pyqtSlot()
    ...
    def max_attempt_QDoubleSpinBox_valueChanged(self):
        ...
        self.save_QPushButton.setStyleSheet('background-color: #FFCC00')
    @pyqtSlot()
    def opacity_QDoubleSpinBox_valueChanged(self):
        self.exp_parameter['opacity'] = self.opacity_QDoubleSpinBox.value()
        self.save_QPushButton.setStyleSheet('background-color: #FFCC00')
    @pyqtSlot()
    ...
    def init_gui(self):
        ...
        self.mask_duration_QLabel = QLabel("Pre-Cue Duration (s):")
        ...
        self.cue_probability_QDoubleSpinBox.setDecimals(2)
        ...
        self.cue_type_QComboBox.addItems(['Arrow', 'Landolt C', 'Both','Circle','Color'])
        ...
        self.show_cue_early_QCheckBox = QCheckBox('Show Cue Early')
        self.sidepanel_params_3_tab_QVBoxLayout.addWidget(self.show_cue_early_QCheckBox)
        ...
        self.keep_cue_on_QHBoxLayout = QHBoxLayout()
        self.keep_cue_on_QLabel = QLabel("Keep Cue On Probability:")
        self.keep_cue_on_QLabel.setAlignment(Qt.AlignRight)
        self.keep_cue_on_QHBoxLayout.addWidget(self.keep_cue_on_QLabel)
        self.keep_cue_on_QDoubleSpinBox = QDoubleSpinBox()
        self.keep_cue_on_QDoubleSpinBox.setValue(0)
        self.keep_cue_on_QDoubleSpinBox.setMaximum(1)
        self.keep_cue_on_QDoubleSpinBox.setSingleStep(0.01)
        self.keep_cue_on_QDoubleSpinBox.setDecimals(2)
        self.keep_cue_on_QHBoxLayout.addWidget(self.keep_cue_on_QDoubleSpinBox)
        self.sidepanel_params_4_tab_QVBoxLayout.addLayout(self.keep_cue_on_QHBoxLayout)
        ...
        self.sidepanel_params_4_tab_QVBoxLayout.addLayout(self.change_prob_QHBoxLayout)
        ...
        self.sidepanel_params_4_tab_QVBoxLayout.addLayout(self.tgt_prob_QHBoxLayout)
        ...
        self.sidepanel_params_4_tab_QVBoxLayout.addLayout(self.max_attempt_QHBoxLayout)
        
        self.opacity_QHBoxLayout = QHBoxLayout()
        self.opacity_QLabel = QLabel("Cue Opacity: ")
        self.opacity_QLabel.setAlignment(Qt.AlignRight)
        self.opacity_QHBoxLayout.addWidget(self.opacity_QLabel)
        self.opacity_QDoubleSpinBox = QDoubleSpinBox()
        self.opacity_QDoubleSpinBox.setValue(1)
        self.opacity_QDoubleSpinBox.setSingleStep(0.1)
        self.opacity_QDoubleSpinBox.setDecimals(2)
        self.opacity_QHBoxLayout.addWidget(self.opacity_QDoubleSpinBox)
        self.sidepanel_params_4_tab_QVBoxLayout.addLayout(self.opacity_QHBoxLayout)
        ...
        self.sidepanel_params_4_tab_QVBoxLayout.addLayout(self.tgt_opacity_QHBoxLayout)
    ...
    def update_parameter(self):
        ...
        cue_types = ['arrow','landolt','both','circle','color']
        ...
        self.show_cue_early_QCheckBox.setChecked(self.exp_parameter['show_cue_early'])
        ...
        self.keep_cue_on_QDoubleSpinBox.setValue(self.exp_parameter['keep_cue_on'])
        ...
        self.opacity_QDoubleSpinBox.setValue(self.exp_parameter['opacity'])
