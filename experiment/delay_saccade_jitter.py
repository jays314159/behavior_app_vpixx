...
class DelaySacEyeProcess(multiprocessing.Process):
    ...
    def run(self):
        ...
        while not self.stop_fsm_process_Event.is_set():
            ...
            #print(f'self_t: {self.t}, rand_signal_t: {random_signal_t}')
            ...
            dout_value = int(DPxGetDoutValue())
            ...
            self.trial_data['dout_value'].append(dout_value)
...
class DelaySacFsmProcess(multiprocessing.Process):
    ...
    def run(self):
        ...
        while not self.stop_fsm_process_Event.is_set():
            if not self.stop_exp_Event.is_set():
                ...
                fsm_parameter['update_time'] = 1 / fsm_parameter['update_rate']
                # Init. var
                self.t = 0
                t_mouse = 0
                old_t = self.t
            ...
            while not self.stop_fsm_process_Event.is_set() and run_exp: 
                ...
                while not self.stop_fsm_process_Event.is_set() and run_exp:
                    ...
                    if state == 'INIT':
                        ...
                        if fsm_parameter['tgt_appear_time'] > 0:
                            opacity = 0
                        else:
                            opacity = 1
                    ...
                    if state == 'DISPLAY_TGT':
                        if not self.eye_blink:
                            ...
                            if fsm_parameter['num_tgt_display'] > 0:             
                                #self.tgt.draw()
                                self.draw_tgt(tgt_symbol,opacity,transparent = tgt_trans_bool)
                    ...
                    if state == 'DELAY_FIXATION':
                        ...
                        if (self.t-state_start_time) >= delay_time:
                            state_start_time = self.t
                            state_inter_time = self.t
                        ...
                        if (self.t - state_inter_time) >= fsm_parameter['update_time']:
                            state_inter_time = self.t
                            tgt_appear_time_ratio = (self.t - state_start_time) / fsm_parameter['tgt_appear_time']
                            if tgt_appear_time_ratio < 1:
                                self.draw_tgt(tgt_symbol,opacity=tgt_appear_time_ratio)
                            else:
                                self.tgt_x = np.random.normal(loc=self.cue_x,scale=fsm_parameter['tgt_jitter_scale'])
                                self.tgt_y = np.random.normal(loc=self.cue_y,scale=fsm_parameter['tgt_jitter_scale'])
                                self.tgt.pos = (self.tgt_x, self.tgt_y)
                                self.draw_tgt(tgt_symbol,opacity=1)
    ...
    def get_trial_eye_data(self):
        ...
        self.trial_data['dout_data'] = eye_data['dout_data']
        self.trial_data['dout_value'] = eye_data['dout_value']
    ...
    def update_target(self,fsm_parameter):
        ...
        self.tgt_fill_color = tgt_parameter['fill_color']
    ...
    def draw_tgt(self,cue_type,opacity=1,change_tgt=False,transparent=False):
        if cue_type == 'cross' and not change_tgt:
            ...
            if transparent:
                ...
                if opacity < 1:
                    fill_color = np.array([0,0,0])*opacity + (1-opacity)*np.array([1.0,1.0,1.0])
                    self.tgt_cross_1.fillColor = fill_color
                    self.tgt_cross_1.lineColor = fill_color
                    self.tgt_cross_2.fillColor = fill_color
                    self.tgt_cross_2.lineColor = fill_color
                else:
                    self.tgt_cross_1.fillColor = self.tgt_fill_color
                    self.tgt_cross_1.lineColor = self.tgt_fill_color
                    self.tgt_cross_2.fillColor = self.tgt_fill_color
                    self.tgt_cross_2.lineColor = self.tgt_fill_color
    ...
    def init_trial_data(self):
        ...
        self.trial_data['cue_pos_x'] = []
        self.trial_data['cue_pos_y'] = []
...
class DelaySacGui(FsmGui):
    ...
    def init_signals(self):
        ...
        self.update_rate_QDoubleSpinBox.valueChanged.connect(self.update_rate_QDoubleSpinBox_valueChanged)
        self.tgt_appear_time_QDoubleSpinBox.valueChanged.connect(self.tgt_appear_time_QDoubleSpinBox_valueChanged)
        self.tgt_jitter_scale_QDoubleSpinBox.valueChanged.connect(self.tgt_jitter_scale_QDoubleSpinBox_valueChanged)
    ...
    def tgt_opacity_QDoubleSpinBox_valueChanged(self):
        ...
        self.save_QPushButton.setStyleSheet('background-color: #FFCC00')
    @pyqtSlot()
    def update_rate_QDoubleSpinBox_valueChanged(self):
        self.exp_parameter['update_rate'] = self.update_rate_QDoubleSpinBox.value()
        self.save_QPushButton.setStyleSheet('background-color: #FFCC00')
    @pyqtSlot()
    def tgt_appear_time_QDoubleSpinBox_valueChanged(self):
        self.exp_parameter['tgt_appear_time'] = self.tgt_appear_time_QDoubleSpinBox.value()
        self.save_QPushButton.setStyleSheet('background-color: #FFCC00')
    @pyqtSlot()
    def tgt_jitter_scale_QDoubleSpinBox_valueChanged(self):
        self.exp_parameter['tgt_jitter_scale'] = self.tgt_jitter_scale_QDoubleSpinBox.value()
        self.save_QPushButton.setStyleSheet('background-color: #FFCC00')
    @pyqtSlot()
    ...
    def init_gui(self):
        ...
        self.update_rate_QHBoxLayout = QHBoxLayout()
        self.update_rate_QLabel = QLabel("Screen Update Rate (fps): ")
        self.update_rate_QLabel.setAlignment(Qt.AlignRight)
        self.update_rate_QHBoxLayout.addWidget(self.update_rate_QLabel)
        self.update_rate_QDoubleSpinBox = QDoubleSpinBox()
        self.update_rate_QDoubleSpinBox.setValue(1)
        self.update_rate_QDoubleSpinBox.setSingleStep(1)
        self.update_rate_QDoubleSpinBox.setDecimals(0)
        self.update_rate_QHBoxLayout.addWidget(self.update_rate_QDoubleSpinBox)
        self.sidepanel_params_4_tab_QVBoxLayout.addLayout(self.update_rate_QHBoxLayout)
        
        self.tgt_appear_time_QHBoxLayout = QHBoxLayout()
        self.tgt_appear_time_QLabel = QLabel("Target Appear Time (s): ")
        self.tgt_appear_time_QLabel.setAlignment(Qt.AlignRight)
        self.tgt_appear_time_QHBoxLayout.addWidget(self.tgt_appear_time_QLabel)
        self.tgt_appear_time_QDoubleSpinBox = QDoubleSpinBox()
        self.tgt_appear_time_QDoubleSpinBox.setValue(1)
        self.tgt_appear_time_QDoubleSpinBox.setSingleStep(0.1)
        self.tgt_appear_time_QDoubleSpinBox.setDecimals(2)
        self.tgt_appear_time_QHBoxLayout.addWidget(self.tgt_appear_time_QDoubleSpinBox)
        self.sidepanel_params_4_tab_QVBoxLayout.addLayout(self.tgt_appear_time_QHBoxLayout)
        
        self.tgt_jitter_scale_QHBoxLayout = QHBoxLayout()
        self.tgt_jitter_scale_QLabel = QLabel("Target Jitter Scale: ")
        self.tgt_jitter_scale_QLabel.setAlignment(Qt.AlignRight)
        self.tgt_jitter_scale_QHBoxLayout.addWidget(self.tgt_jitter_scale_QLabel)
        self.tgt_jitter_scale_QDoubleSpinBox = QDoubleSpinBox()
        self.tgt_jitter_scale_QDoubleSpinBox.setValue(1)
        self.tgt_jitter_scale_QDoubleSpinBox.setSingleStep(1)
        self.tgt_jitter_scale_QDoubleSpinBox.setDecimals(2)
        self.tgt_jitter_scale_QHBoxLayout.addWidget(self.tgt_jitter_scale_QDoubleSpinBox)
        self.sidepanel_params_4_tab_QVBoxLayout.addLayout(self.tgt_jitter_scale_QHBoxLayout)
    ...
    def update_parameter(self):
        ...
        self.update_rate_QDoubleSpinBox.setValue(self.exp_parameter['update_rate'])
        self.tgt_appear_time_QDoubleSpinBox.setValue(self.exp_parameter['tgt_appear_time'])
        self.tgt_jitter_scale_QDoubleSpinBox.setValue(self.exp_parameter['tgt_jitter_scale'])
