...
class DataManager(QRunnable):
    ...
    def save_data(self, trial_num, trial_data):
        '''
        save data to a specified HDF5 file
        '''

        # data_file = h5py.File(self.data_file_path +'.hdf5','a',libver='latest')
        group_name = "trial_"+str(trial_num)
        ...
        #         trial_data[trial_key] = np.append(trial_grp[trial_key][:], trial_data[trial_key])
        #     del data_file[group_name]
        # except:
        #     pass
        # finally:
        trial_grp = self.data_file.create_group(group_name)       
        for key,value in trial_data.items():
            trial_grp.create_dataset(key,data=trial_data[key])
        # data_file.close() 
    ...
    def init_data(self, exp_name, exp_parameter):
        ...
        self.data_file = h5py.File(self.data_file_path+'.hdf5','a',libver='latest')
        for key,value in exp_parameter.items():
            self.data_file.attrs[key] = value
        # data_file.close() 
